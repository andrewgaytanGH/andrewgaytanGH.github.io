...
Since we are predicting a numeric output, we tested three regression modeling approaches:
...
Testing indicated that the Random Forest had the highest predictive accuracy.
...
While predictive accuracy was relatively high, other modeling approaches could be tested, especially models similar to Random Forest, such as XGBoost or LightGBM, to determine whether further accuracy gains are possible.

From a data perspective, further variables could be collected, and further feature engineering could be undertaken to ensure that as much useful information as possible is available for predicting customer loyalty
<br>
<br>
...
| product_area_count | Independent | The number of product areas within ABC Grocery the customer has shopped into within the latest 6 months |
...
We will build a model to accurately predict the "loyalty_score" metric for those customers that were able to be tagged, based upon the customer metrics listed above.

If successful, we can use this model to predict the customer loyalty score for the customers that were unable to be tagged by the agency.
...
# instantiate RFECV & the model type to be used
...
We will again use the scikit-learn library within Python to model our data using a Decision Tree. The code sections below are broken up into 4 key sections:
...
Instantiating and training our Decision Tree model is done using the below code. We use the *random_state* parameter to ensure reproducible results, and this helps us understand any improvements in performance with changes to model hyperparameters.
...
<br>
### Visualise Our Decision Tree <a name="regtree-visualize"></a>
...
<br>
This visual can be shared with business stakeholders to help explain the factors driving the predictions.

One notable finding is that the *very first split* appears to be using the variable *distance from store* suggesting that this is a very important variable when it comes to predicting loyalty!

___
<br>
# Random Forest <a name="rf-title"></a>

We will again use the scikit-learn library within Python to model our data using a Random Forest. The code sections below are broken up into 4 key sections:
...
Instantiating and training our Random Forest model is done using the below code. We use the *random_state* parameter to ensure reproducible results, and this helps us understand any improvements in performance with changes to model hyperparameters.
...
As we did when testing Linear Regression & our Decision Tree, we will again use Cross Validation (for more info on how this works, please refer to the Linear Regression section above)
...
We can apply both approaches and plot the results.
...
```
<br>
That code produces the plots below: the first for *Feature Importance* and the second for *Permutation Importance*.
...
<br>
The overall interpretation from both approaches is very similar, in that by far, the most important or impactful input variable is *distance_from_store* which is the same insights we derived when assessing our Linear Regression & Decision Tree models.
...
<br>
Although the primary objective was predictive accuracy rather than interpretability, it was useful to observe across all three modeling approaches, that the input variable with the biggest impact on the prediction was *distance_from_store* rather than variables such as *total sales*. This provides useful business context beyond the prediction task itself.
...
```
<br>
Using this process, we generated *loyalty_score* predictions for customers with missing values. Given the model's test-set performance, these predictions provide a reasonable basis for downstream analysis. This extra customer information will ensure our client can undertake more accurate and relevant customer tracking, targeting, and communications.
...
We could also tune the hyperparameters of the Random Forest, notably regularization parameters such as tree depth, as well as potentially training on a higher number of Decision Trees in the Random Forest.
